...
__global__ void startClustering(float *study_time, float *examPerformance, float *targetAnswers,int N, int svHX, int svHY, int svLX, int svLY, int seedDistance, int lowCluster, int highCluster, int cluster_one_index, int cluster_two_index, struct cluster *cluster_one, struct cluster *cluster_two)
{
	...
	distance_1 = abs(svHX-study_time[idx]) + abs(svHY-examPerformance[idx]);
	distance_2 = abs(svLX-study_time[idx]) + abs(svLY-examPerformance[idx]);
	
	if((distance_1 <= seedDistance) && (highCluster==1))
	{
		cluster_one[idx].study_time = study_time[idx];
		cluster_one[idx].exam_performance = examPerformance[idx];
		cluster_one[idx].target_value = targetAnswers[idx];
	}
	else if((distance_2 <= seedDistance) && (lowCluster==1))
	{
		cluster_two[idx].study_time = study_time[idx];
		cluster_two[idx].exam_performance = examPerformance[idx];
		cluster_two[idx].target_value = targetAnswers[idx];
	}
}
